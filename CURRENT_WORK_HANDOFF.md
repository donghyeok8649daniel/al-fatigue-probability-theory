...
## 2026-09-25 Si 개시 v12: 배터리 최대 절전 중단 후 정리

- 사용자 요청은02:40:54-10:40:54KST의8시간 보완 연구였다. Windows 전원 이벤트가
  05:08:49 Battery 최대 절전/18:02:21 복귀를 확인한다. **연속8시간 완료 아님.**
  복귀 때 계산 예산과 마감 guard가 작동해 원자 계산·대기 프로세스는 종료됐다.
  새 장시간 원자 계산을 시작하지 말고 사용자의 새 재개 지시를 따른다.
- 기준86360a0에서Si 브랜치만 작업했다. Al/UI/생산 에너지·Hz gate 보존.
  `results/silicon_initiation_v12/COMPLETED_SUMMARY.md`, WORKING_STATUS.md,
  RESULTS_AND_LIMITS.md, session_interruption.json, execution_ledger.json 우선.
- 실제:100MPa전체649Hessian/독립6방향검증 완료, 최소0.02713787eV/A2.
  8%는539/648행 부분; 전체 안정성/최소mode 미확보. 복귀8%/10% 및 후속
  원자평가는미실행. elapsed50,464초를연속CPU시간으로보고하지않는다.
- 총55에너지·힘forward,1192자동미분행,12보존적fit,새DFT0/MD0.
  두Si-O보정은QE SiO2에서악화해미채택. 실제Si개시basin·장벽·시간·수명미검증.
- 작은율의확률누출반례6.8666%를수정. v9감사재실행5파일byte동일.
  최종63PASS/5.16s/2ASE경고,14시험소스해시결합. 전하/공간경계 합성정확해
  검증은실제Si·도펀트 농도·물리clock보정이아니다.
- 큰산소포함69구조/MPA199표본/0축력복귀/비선형검사는코드만준비했다.
  비교가중치다운로드를실제평가라고부르지않는다. 부분행resume는아직지원안됨.
- 기존si-7은이전PAUSED상태. 새automation도구가없어예약하지않았다.
  전원정책/배터리보호설정은바꾸지않았다. 후속연구는전원연결유지가필요하다.
- 보고서는15쪽으로완료근거와중단기록을구분한다. PDF생성marker는이미1회
  실행했으니반복금지. 최종파일/QA/Gitrevision은최종전달과PDFmanifest를따른다.

## 아래 v11 기록은 과거 마감 상태

## 2026-09-24 Si 균열 개시 v11 계산·감사 종료

- 예약 원자 계산과 독립 후처리는07:42KST까지 모두 종료했다. 재실행하지 않는다.
  `results/silicon_initiation_v11/COMPLETED_SUMMARY.md`와 `RESULTS_AND_LIMITS.md` 우선.
- QE1159/CP2K1013 원자료 평가·독립 재검증, 화학환경 오차, 원문27대조 완료.
  산소이웃1~3 Si 힘 오차가 약0.572eV/A로 커서 재료 미채택이다.
- bare360Si 변위0~8%와 별도10% 이완,10->8% 감하중이 수렴했다. 같은8%에서
  응력8.624->2.234GPa/에너지차-9.719eV/원자 matching RMS1.054A다.
  큰 정적 이력 의존성을 실제 소성·균열 개시·피로소산으로 인증하지 않는다.
- 축력0/100MPa 정밀 검사와 독립 raw replay 완료. 곡률Krylov16/64는 모두
  예산 종료/미수렴/저장mode0. 힘 수렴과 국소 안정성의 차이를 유지한다.
- 확률 보존의 시간단위 의존 결함 수정, 관련41PASS와 시험 소스 SHA 보존.
  최종 source/artifact manifest 및 Git blob 검증 방법은 REPRODUCE.md를 따른다.
- 보고서 builder는16쪽이며 최종 PDF는 별도 산출 폴더에 둔다. PDF의 실제
  사용 커밋·파일 SHA는 동반 manifest로 식별한다. 검증된 Si 개시 장벽/확률/수명은 없다.
- 과거 성장 관련 계산과 v10 강체분리 자료는 보존한다. Al/UI/생산 Hz 불변.

## 2026-09-24 최신 지시: 균열 개시와 열산화층, 약7시간 자율 연구

- 사용자가 v2/v3/v4/v6의 성장 관련 결과를 개시로 취급한 방향을 정정했다.
  후속 주목표는 초기 균열을 넣지 않은 상태에서 첫 균열 형성이다.
  외부 표면/제조 결함/도핑 상태는 명시한다. 검증된 개시 장벽/확률/수명은 없다.
- 제공 thermal-oxide 논문을 근거로 v11을 시작한다. 실제 계획/진행/마감은
  results/silicon_initiation_v11/WORKING_STATUS.md를 먼저 읽는다.
- v2 .703eV와 v6 약110MPa는 개시 결과가 아니다. v10도 강체 계면 분리다.
  v4 미완료 계산이나 이전 K 경계 계획을 자동 재개하지 않는다.
- 시작 Si local/fresh origin HEAD736bcf5494871b483dfcc4f20fee54e509099ff6 일치/clean.
  추가agent 없이 진행하며 원래 Al/UI 작업은 보존한다.
...
- 최신 요청: Al에서 외삽 숫자·곡선·메시 위험도가 모두 안 보임.
  실제 열린 UI는 native pipe 오류로 읽지 못했다. 별도 실제 Tk UI에서
  3D 실행 중 record callback 누락과 프로젝트 복원 시 외삽 갱신 누락을 재현했다.
- 3D 주 곡선과 같은 첫 대표 PDE 기록을 UI로 전달하며 다른 대표를 이어 붙이지 않는다.
  저장 파일 복원 시 원래 국소 확률과 복원한 면적 입력으로 외삽을 재계산한다.
  면적 입력 안내 위치를 Solve로 바로잡았다. PDE/인증/재료 gate는 불변이다.
- 전체 app: 188 PASS+3subtests, PC 절전·최대절전 중 deadline 초과 1건.
  같은 node 재검증 PASS, 고유189개 최종 통과 대조 완료. 원래 실행 폴더에도
  공통 변경8파일만 적용하고 집중16 PASS/91.73s. desktop smoke exit0.
  자세한 재현·검증·제한은 `results/ui_extrapolation_fix/COMPLETED_SUMMARY.md`.
- 열린 기존 프로세스에는 자동 적용되지 않는다. 사용자 작업을 저장하고 재실행해야 한다.
  면적 외삽에는 A_c/A_stressed, 체적 위험도에는 V_c가 필요하다. 공간 지도는
  실제 공간 계산 완료 후 표시하며 미계산 영역이나 임의 상관체적을 만들지 않았다.
  사용자 현재 창의 정확한 미표시 조건을 확인했다고 주장하지 않는다.
