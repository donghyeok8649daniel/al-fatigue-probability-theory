...
PURPOSES = {
    ...
    "simulations/fem_geometry_mesh.py": "1D normal-only 결과를 표시할 실제 1D/2D/3D mesh를 생성하고 CAD/mesh 파일을 읽는 geometry 계층이다.",
    "simulations/fem_mesh_ui.py": "실제 1D/2D/3D cell connectivity와 normal-only scalar field를 mesh 경계 위에 시각화한다.",
    "simulations/fem_mesh_app.py": "1D/2D/3D FEM geometry mesh 생성·CAD 입력·가시성 조절을 제공하는 경량 Matplotlib UI다.",
    "simulations/run_tensile_mesh_projection.py": "1D FEM/확률 이력의 normal-only scalar를 실제 1D/2D/3D 또는 CAD mesh cell에 매핑해 저장한다.",
    ...
    "tests/test_fem_geometry_mesh.py": "1D/2D/3D mesh 생성, CAD surface 입력, normal projection과 axial field mapping을 회귀검증한다.",
    "tests/test_fem_mesh_projection.py": "1D element history를 실제 1D/2D/3D mesh cell field로 내보내는 통합 경로를 검증한다.",
    "tests/test_fem_mesh_app.py": "경량 FEM mesh UI의 1D/2D/3D 생성, clipping 및 headless rendering helper를 검증한다.",
