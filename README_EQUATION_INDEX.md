...
1. [`docs/EQUATION_SUMMARY_1D_P_U_THETA.md`](docs/EQUATION_SUMMARY_1D_P_U_THETA.md)  
   **Equation sheet / 핵심 수식 정리본** — active governing equations from the LJ chain to exact integral representations and first passage.
2. [`docs/VARIABLE_INDEX_1D_P_U_THETA.md`](docs/VARIABLE_INDEX_1D_P_U_THETA.md)  
   **Bilingual symbol dictionary / 영·한 기호 사전** — every active symbol must carry an equation definition, mathematical meaning, physical meaning, unit/scale, and status.
3. [`docs/MASTER_1D_P_U_THETA_FORMULATION.md`](docs/MASTER_1D_P_U_THETA_FORMULATION.md)  
   Full differential derivation / 전체 미분형 유도.
4. [`docs/MILESTONE25_EXACT_INTEGRAL_REPRESENTATION.md`](docs/MILESTONE25_EXACT_INTEGRAL_REPRESENTATION.md)  
   Exact push-forward, Volterra, characteristic, and survival integral representations / 정확한 적분해 표현.
5. [`docs/CRACK_INITIATION_DEFINITION.md`](docs/CRACK_INITIATION_DEFINITION.md)  
   Kinetic first-passage definition / 위상공간 최초통과 균열개시 정의.

---

# 2. Mandatory symbol-definition contract / 기호 정의 강제 규칙

From this point onward, **a new mathematical symbol is not considered defined in the theory unless the symbol index is updated at the same time.**
앞으로는 **새 수학기호를 도입하면서 기호 Index를 동시에 갱신하지 않으면 이론상 정의된 기호로 인정하지 않는다.**

Every new symbol entry MUST include all of the following:
...
| English term | standard English name used in the paper | 논문에서 사용할 영문 명칭 |
| Korean term | Korean name used in notes/reports | 한국어 명칭 |
| Mathematical definition | domain, conditioning, measure, operation, tensor/scalar role, etc. | 정의역, 조건부 조건, 측도, 연산, 스칼라/텐서 역할 등 |
...
| Dependencies | symbols/equations needed before this symbol is meaningful | 이 기호를 정의하기 위해 먼저 필요한 기호/식 |

A prose-only definition is insufficient when a mathematical defining relation exists.
수식으로 정의 가능한 기호는 **문장 설명만으로 정의하지 않는다.**

A symbol must not silently change meaning between files.
동일 기호의 의미를 파일마다 임의로 바꾸지 않는다.

If the same glyph would be reused for a different mathematical object, one of the symbols must be renamed or explicitly decorated/indexed.
같은 글리프가 다른 수학적 객체를 뜻하게 되면 기호를 바꾸거나 첨자/장식을 붙여 구분한다.

---

# 3. Active mathematical chain / 활성 수학 체계
...
The finite LJ dynamics is closed.  The one-point $P$–$u$–$\Theta$ PDEs are exact but hierarchical.  The same reduced fields also possess exact full-flow integral representations.
유한 LJ 사슬은 닫힌 동역학계이고, $P$–$u$–$\Theta$ 축약 PDE는 정확하지만 계층적이다. 동시에 동일한 축약장은 전체 미시 흐름의 정확한 적분 투영으로 표현된다.

---

# 4. Governing-equation index / 지배방정식 Index
...
## E04 — projected kinetic transport / 투영된 위상공간 수송
...
## E07 — corrected exact $\Theta$ equation / 수정된 정확한 $\Theta$ 식
...
# 5. Symbol groups / 기호 그룹

The complete entries live in `docs/VARIABLE_INDEX_1D_P_U_THETA.md`.
전체 상세 정의는 `docs/VARIABLE_INDEX_1D_P_U_THETA.md`를 기준으로 한다.
...
| marginal density | 주변밀도 | $P=\int Fdc$ | spacing distribution after velocity information is projected out |
| conditional mean | 조건부 평균 | $u=E[c\mid\lambda]$ | mean opening/closing rate at fixed spacing |
| conditional variance | 조건부 분산 | $\Theta=\operatorname{Var}(c\mid\lambda)$ | unresolved spread of spacing rates at fixed spacing |
| conditional acceleration | 조건부 가속도 | $\mathcal A=E[\ddot\lambda\mid\lambda]$ | mean microscopic acceleration at a given spacing |
| acceleration covariance | 가속도 공분산 | $\Psi=\operatorname{Cov}(c,\ddot\lambda\mid\lambda)$ | coupling between rate fluctuations and acceleration fluctuations |
| moment hierarchy | 모멘트 계층 | $\partial_\tau R_r+\partial_\lambda R_{r+1}=rB_r$ | information lost when projecting the full phase-space state |
...
| Volterra integral equation | 볼테라 적분방정식 | present state expressed using past-time integrals | exact time-history form of the reduced balances |
...
| closure | 폐쇄/클로저 | expression of higher statistics using retained fields only | extra assumption needed for an autonomous reduced solver |
