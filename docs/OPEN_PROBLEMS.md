...
4. Can a free surface, geometry, or normal stress concentration produce localized opening without an empirical damage variable?
5. How does a finite-temperature microscopic ensemble alter tail transport in $P(a,t)$?
6. Does any observed cycle-to-cycle drift survive time-step, system-size, and boundary-condition refinement?

## Milestone 3 — Time-scale bridge to laboratory fatigue frequencies

Direct atomic LJ dynamics evolves on an atomic time scale. Therefore high-frequency lattice-dynamic instability is not by itself a 20 Hz fatigue prediction.
...
The active crack-initiation picture is normal opening or normal bond instability. For the generalized-LJ baseline, the local idealized instability is
...
4. 경험적 damage variable 없이 자유표면, 형상 또는 normal stress concentration이 국부 opening을 만들 수 있는가?
5. finite-temperature microscopic ensemble이 $P(a,t)$의 tail transport를 어떻게 바꾸는가?
6. 관찰된 cycle-to-cycle drift가 time-step, system-size, boundary-condition refinement 후에도 남는가?

## 마일스톤 3 — 실험 피로주파수로의 시간척도 연결

직접적인 atomic LJ dynamics는 원자 시간척도에서 움직인다. 따라서 고주파 lattice-dynamic instability 자체는 20 Hz 피로예측이 아니다.
...
출발점은 microscopic Liouville dynamics, conditional propagator, projection으로부터 유도한 memory kernel이다. Markovian reduction을 사용하려면 이 수준에서 정당화되어야 하며 처음부터 가정하면 안 된다.
