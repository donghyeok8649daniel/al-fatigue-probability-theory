# Milestone 17 — 2D/3D Geometry Mesh and CAD Input with a 1D Normal-Only Theory

## Status

**ACTIVE GEOMETRY/MESH INTERFACE — NOT A 2D/3D ELASTICITY OR MULTIAXIAL FATIGUE SOLVER**

This milestone permits two- and three-dimensional specimen meshes without changing the active microscopic theory. The dimensionalities are deliberately separated:
...
| STEP/STP, IGES/IGS, BREP | optional Gmsh/OpenCASCADE | 2D or 3D generated mesh | requires `requirements-cad.txt` |
...
# 한국어 번역 — 1D normal-only 이론을 유지한 2D/3D 메시 및 CAD 입력

## 상태

**활성 geometry/mesh 인터페이스 — 2D/3D 탄성 또는 다축 피로 solver가 아님**

이 마일스톤은 활성 microscopic theory를 바꾸지 않고 2차원·3차원 시편 메시를 허용한다. 두 차원은 의도적으로 분리한다.
...
기존의 단순 graphical extrusion과 달리 각 cell에는 명시적인 node connectivity가 있다. 3D rendering 전에는 공유 interior face를 제거하고 boundary face만 추출하며, 2D cell은 모두 표시한다. mesh cell 수는 numerical discretization count이며 통계적으로 독립인 microscopic region 수와 동일하지 않다.
...
| STEP/STP, IGES/IGS, BREP | 선택 Gmsh/OpenCASCADE | 생성된 2D 또는 3D mesh | `requirements-cad.txt` 필요 |
