...
def main(args):
    ...
    max_atoms=int(summary['protocol']['max_atoms']);seen=[];excluded=[];refs={};groups={};worst=[]
    ...
    for index,d in enumerate(frames(args.source)):
        inv=inventory[index];n=len(d['numbers']);selected=n<=max_atoms
    ...
    result=dict(complete=True,source_sha256=SOURCE_SHA,source_frames=index+1,frames_verified=len(seen),
        atoms_verified=total_atoms,excluded_frame_ids=excluded,max_atoms=max_atoms,
