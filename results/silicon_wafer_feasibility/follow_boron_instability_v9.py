...
def main(args):
    ...
    for sign,label in [(1,'plus'),(-1,'minus')]:
        ...
        attempts=[]
        opt=BFGSLineSearch(a,logfile=str(args.output/(key+'_bfgs_line.log')),maxstep=.08)
        history=[]
        opt.attach(lambda:history.append(float(a.get_potential_energy())),interval=1)
        ok=opt.run(fmax=1e-5,steps=300)
        attempts.append(dict(method='BFGSLineSearch',steps=opt.nsteps,converged=bool(ok),
            maximum_logged_energy_increase_eV=float(max(np.diff(history))) if len(history)>1 else None,
            force_max_eV_A=float(np.max(np.linalg.norm(a.get_forces(),axis=1))),energy_eV=float(a.get_potential_energy())))
        write(args.output/(key+'_bfgs_line.extxyz'),a)
        if not ok:
