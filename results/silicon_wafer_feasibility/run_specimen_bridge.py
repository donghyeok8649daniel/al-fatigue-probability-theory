...
def source_cutoff(style, potential):
    """Read the pure-Si cutoff from the bound source, never a guessed label."""
    words = ' '.join(line.split('#')[0] for line in Path(potential).read_text().splitlines()).split()
    if style == 'sw':
        if words[:3] != ['Si']*3 or len(words) != 14:
            raise ValueError('unexpected SW source')
        return float(words[4])*float(words[5])
    if style == 'tersoff':
        entries = [words[i:i+17] for i in range(0, len(words), 17)]
        matches = [r for r in entries if r[:3] == ['Si']*3 and len(r) == 17]
        if len(matches) != 1:
            raise ValueError('one complete Si-Si-Si Tersoff entry required')
        return float(matches[0][13])+float(matches[0][14])
    raise ValueError('unsupported potential style')
...
def load_case(engine, elastic, boundary, K, initial, output, *, tolerance=2e-6):
    ...
    if output.suffix != '.npz':
        raise ValueError('provide a complete .npz filename; never infer a suffix from a decimal load label')
    if output.exists():
        raise FileExistsError('refusing to replace an existing atomic state')
    ...
    np.savez_compressed(output, positions=positions, prescribed=prescribed,
...
def main():
    ...
    for label, old in baseline['models'].items():
        ...
        with LammpsSilicon(md['style'], potentials[label], sha256=md['potential_sha256']) as engine:
            ...
            cutoff = source_cutoff(md['style'], potentials[label])
            for radius in args.radii:
                ...
                for factor in args.factors:
                    ...
                    positions, row = load_case(engine, elastic, boundary, k, previous, args.output/(name+'.npz'))
