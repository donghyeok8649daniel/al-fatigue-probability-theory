"""Continue one interrupted fixed-displacement state from full-precision raw data.

The saved geometry and grip displacement are immutable; this introduces no crack
seed, extra strain, or new perturbation. Old run files are never overwritten.
"""
...
def main(args):
    ...
    extension=float(np.mean(positions[upper,2]-reference[upper,2])-np.mean(positions[lower,2]-reference[lower,2]))
    target=prescribed_grips(reference,lower=lower,upper=upper,extension_A=extension)
    fixed=lower|upper
    if np.max(abs(positions[fixed]-target[fixed]))>1e-12:
    ...
    protocol=dict(model_sha256=MODEL_SHA256,source_raw_sha256=hashlib.sha256(args.state.read_bytes()).hexdigest(),
        ...
        displacement_changed=False,new_initial_perturbation=False,initial_crack_seed_added=False,
        scope='continuation of one saved fixed-displacement state, optimizer memory restarted',
        ...
        for method in ('LBFGSLineSearch','FIRE'):
            ...
            def checkpoint():
                ...
                if time.perf_counter()-started>=args.max_seconds:
                    raise TimeoutError('explicit continuation wall-time budget')
    ...
    grip_error=float(abs(atoms.positions[fixed]-positions[fixed]).max())
    if grip_error>1e-12:raise ValueError('fixed grips moved during continuation')
    ...
    row=dict(converged=converged,complete=converged,interruption=interruption,attempts=checks,
        ...
        energy_change_during_relaxation_eV=energy-saved_energy,grip_coordinate_error_A=grip_error,
    ...
    if not converged:raise RuntimeError('continuation did not converge; raw state preserved')
