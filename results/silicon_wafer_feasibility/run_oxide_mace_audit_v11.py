...
def parse_source(path):
    ...
    for part in raw.split('BEGIN_CFG')[1:]:
        ...
        # Source frame575 has a nonsingular left-handed lattice basis. This is
        # a valid periodic cell; handedness is not a negative physical volume.
        if cell.shape!=(3,3) or abs(np.linalg.det(cell))<=1e-12 or not np.all(np.isfinite(forces)):
...
def main(args):
    ...
    protocol=dict(source_sha256=SOURCE_SHA,source_commit=SOURCE_COMMIT,
        ...
        left_handed_source_frames=[i for i,f in enumerate(frames) if np.linalg.det(f['cell'])<0],
    ...
    with (args.output/'frames.csv').open('w',newline='',encoding='utf-8') as stream:
        ...
        for i,frame in enumerate(frames):
            ...
            if i in (0,26,66,163,164,500,575,800,1158):
            ...
            if np.linalg.det(frame['cell'])<0:
                equivalent=atoms.copy(); equivalent.cell[0]*=-1
                e_equiv,f_equiv=force_only(model,equivalent)
                cell_control=dict(frame=i,basis_change='negate first lattice vector; identical periodic lattice',
                    energy_difference_eV=e_equiv-energy,force_max_difference_eV_A=float(np.max(abs(f_equiv-force))))
                dump(args.output/'left_handed_cell_control.json',cell_control)
                if abs(e_equiv-energy)>1e-8 or np.max(abs(f_equiv-force))>1e-8:
                    raise RuntimeError('equivalent lattice basis check failed')
    ...
    dump(args.output/'summary.json',dict(complete=True,frames=len(rows),atoms=sum(x['atoms'] for x in rows),
        elapsed_s=time.perf_counter()-started,force_only_calls=len(rows)+sum(int(np.linalg.det(f['cell'])<0) for f in frames),standard_controls=controls,
