...
def main(args):
    ...
    for index, strain in enumerate(args.strains):
        ...
        interrupted = None
        try:
            for method in ('LBFGS', 'FIRE'):
                opt = (LBFGS(atoms, logfile=str(folder/'lbfgs.log'), maxstep=.06)
                       if method == 'LBFGS' else
                       FIRE(atoms, logfile=str(folder/'fire.log'), maxstep=.04, dt=.02))
                def checkpoint():
                    if opt.nsteps % 20 == 0:
