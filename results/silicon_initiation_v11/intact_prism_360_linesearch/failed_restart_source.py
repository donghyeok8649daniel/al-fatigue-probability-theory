...
def main(args):
    ...
    if args.initial is not None:
        prior=read(args.initial)
        if (not np.array_equal(prior.numbers,atoms.numbers)
                or not np.allclose(prior.positions[fixed],reference[fixed],atol=1e-9,rtol=0)):
            raise ValueError('restart is not the same intact-prism geometry and zero-load grips')
        atoms.positions[:]=prior.positions
