...
def load_frame(path,metadata):
    with np.load(path) as d:
        ...
        energy,force=correction_features(numbers,positions,cell,pbc)
        return dict(metadata=metadata,energy_basis=energy,force_basis=force.reshape(-1,8),
...
def fit(frames):
    ...
    if rank!=8:raise ValueError('radial correction rank deficient; no regularized candidate accepted')
...
def main(args):
    ...
    protocol=dict(selection='all 487 condensed Si/O/H or Si/O CP2K frames already evaluated in v11',
        ...
        coefficients=8,mace_weights_changed=False,production_enabled=False,material_approved=False,
    ...
    frames=[];manifest=[]
    for row in eligible:
        index=int(row['source_frame']);f=load_frame(args.cp2k/'raw'/f'{index:04d}.npz',row)
    ...
    for row in qe_metadata:
        index=int(row['frame']);f=load_frame(args.qe/'raw'/f'{index:04d}.npz',row)
