...
사용자의 정정에 따라 균열을 미리 넣지 않은 초기 시편의 첫 균열 형성을
주목표로 한다. v2의 seeded crack 선단 전이, v3/v4의 같은 상태 열운동,
v6의 K 경계는 성장 관련 보조 연구다. v10 강체 분리도 개시 장벽이 아니다.
아래 역사적 구현 상태를 검증된 개시 확률/수명으로 읽지 않는다.
최신 진행은 [v11 기록](results/silicon_initiation_v11/WORKING_STATUS.md)과 인계를 읽는다.

v11 계산·원자료 감사의 종료 결과는 [완료·실패·한계](results/silicon_initiation_v11/COMPLETED_SUMMARY.md)에 있다.
공개 DFT 대조와 초기 균열 없는360Si 정적 인장/되돌림을 실행했지만 실제 개시 장벽·
확률·물리시간은 미검증이다. 정밀 국소 곡률 두 실행도 미수렴으로 보존했다.
