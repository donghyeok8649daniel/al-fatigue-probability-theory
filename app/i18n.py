...
_ROWS = (
    ...
    ("load.scope", "메시 후 하중을 적용할 면과 법선·전단 응력을 준비합니다. 현재 체적 FVM/FEM과 국소 P(a,s,t) PDE는 연결되지 않았으므로, 이 조건은 해석 준비 데이터로만 저장됩니다.", "Prepare the loaded face and normal/shear tractions after meshing. Volume FVM/FEM and local P(a,s,t) PDE are not connected, so these conditions are stored as preparation data only."),
    ...
    ("load.prepared", "조건 저장됨: {faces}개 면. 현재 PDE에는 자동 전달하지 않습니다.", "Condition stored: {faces} faces. Not forwarded to the current PDE."),
