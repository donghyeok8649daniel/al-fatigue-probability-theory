...
_ROWS = (
    ...
    ("load.balance_scope", "저장 하중은 중첩 합산됩니다. 응력은 전역 XYZ 성분이며 견인력은 σ·n입니다. 보정은 선택면에 분포 견인력을 추가합니다. 지지 반력이나 구조해석 결과가 아닙니다.", "Stored loads add, including overlaps. Stress uses global XYZ; traction is σ·n. Correction adds distributed traction on selected faces; it is not a support reaction or a structural solution."),
    ("load.balance_confirm", "선택면에 최소 제곱 견인력 보정을 추가할까요? 매 시각 합력·토크를 상쇄하며 기존 응력 행렬은 변경하지 않습니다.\nt=0 [Fx,Fy,Fz (N), Tx,Ty,Tz (N mm)]\n전: {before}\n후: {after}\n하중/메시 변경 시 보정은 해제됩니다.", "Add minimum-norm traction correction on selected faces? It cancels force/torque at every time without changing stress matrices.\nt=0 [Fx,Fy,Fz (N), Tx,Ty,Tz (N mm)]\nBefore: {before}\nAfter: {after}\nChanging loads/mesh clears correction."),
    ("load.balance_done", "평형 보정 승인됨: 모든 시각에 적용, 생산 공간솔버 미연결", "Balance approved: applied at all times; not connected to a production spatial solver"),
    ...
    ("load.unsupported_solver", "생산 국소 PDE는 축응력만 지원합니다. 다중 면하중·평형보정·임의 텐서의 공간 해석은 미연결이므로 이 셋업을 해석하지 않습니다.", "The production local PDE supports axial stress only. Multiple face loads, balancing and custom tensor spatial mechanics are not connected; this setup is not solved."),
    ...
    ("load.prepared", "{faces}개 면 하중을 저장했습니다. 공간 역학 해석 결과가 아니며 생산 국소 PDE 연결은 별도 제한됩니다.", "Stored load on {faces} faces. This is not a spatial mechanics solution; production local-PDE compatibility is separately restricted."),
