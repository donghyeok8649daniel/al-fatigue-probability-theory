...
The probability panels use only the 64 trajectories in the current native solve. Cumulative initiation is one minus the surviving fraction, discrete hazard is the newly absorbed mass divided by the preceding survivor mass, and life probability mass is the observed drop in survival at each native first-passage cycle. No external TMW scale, S--N dataset, named life distribution, kernel, or fitted tail is used.

The native solver is a dimensionless mechanism solver. Its applied-load cycle count and model time are displayed as such; they are not relabeled as calibrated laboratory seconds or pure-aluminum fatigue life. A quantitative laboratory bridge remains unresolved in the theory repository and must not be silently supplied by the desktop layer.
