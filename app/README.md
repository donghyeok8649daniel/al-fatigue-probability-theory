...
The default initial measure is the explicitly declared ideal state $\mu_0=\delta_{(\lambda=1,c=0)}$. The one-point phase-space density uses the exact $1/M$ spatial counting measure over chain spacings, while specimen survival and first-passage mass use the declared $\mu_0$ weights. They are not Monte Carlo frequencies and have no binomial confidence interval. A nontrivial specimen probability requires a physically declared broader $\mu_0$, which remains open in the theory. No external S--N dataset, named life distribution, kernel, or fitted tail is used. Once an initial-state atom first crosses $\lambda_c$, it is removed from survivor-conditioned observables; when no specimen mass remains, integration stops rather than plotting post-failure strain runaway.
