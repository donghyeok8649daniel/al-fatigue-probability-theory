...
\usepackage{amsmath,amssymb,bm,booktabs,longtable,array,hyperref}
\title{Exact Two-Row Registry Energy and an Active Ideal-Slip Extension}
...
\begin{center}\fbox{\parbox{0.9\textwidth}{\textbf{Historical note (2026-09-02).}
The active theory now uses this exact row expression only as $W(d,s)$ in the
multiplicity-free sum $U_0(a,s)=\sum_{k\ge1}W(ka,s)$. This file preserves the
earlier single-row audit and is not the current governing paper.}}\end{center}
...
event. The corrected energy and unwrapped scalar-registry dynamics are an
optional active ideal mechanism. Quantitative plasticity of aluminum is not
claimed.
\end{abstract}

\section{Scope and status}

The primary crack-initiation branch remains the one-dimensional normal model
with state coordinate $a$ and density $P(a,t)$. This note supplies a separate
optional active branch with one scalar registry $s$ and unwrapped well index
$z$. It is still a one-index row lattice and one-slip-coordinate mechanism;
it is not a multiaxial continuum law. No FEM element currently supplies shear
stress to this branch.
...
\section{Active probability implementation}

The active reduced solver prescribes $a/b$, puts $u=s/b$, and evolves the
unwrapped density by
\begin{equation}
 \partial_t p=-\partial_u j,\qquad
 j=-\left[(\partial_u w-g)p+\beta^{-1}\partial_u p\right],
 \quad w={W\over\epsc},\quad
 g={\tau A_{\rm rep}b\over\epsc}.
\end{equation}
Here time is scaled by $b^2/(M_s\epsc)$ and
$\beta=\epsc/(\kB T)$. Chang--Cooper finite-volume fluxes and backward Euler
preserve probability and nonnegativity. The initial state is a Boltzmann
density conditioned on one registry well; a global density on the infinite
periodic line is not normalizable.

Plastic residual is not inferred from a crossing alone. After unloading and a
declared relaxation time it requires
\begin{equation}
 \langle\widetilde s\rangle\longrightarrow0,
 \qquad \langle z\rangle\ne0,
 \qquad \gamma_p={b\over h_{\rm slip}}\langle z\rangle.
\end{equation}
The current subcritical dimensionless pulse calculation gives
$\langle z\rangle=0.4913473$ and
$\langle\widetilde s/b\rangle=1.59\times10^{-10}$ after unloading, while six
symmetric zero-mean cycles give $\langle z\rangle=0.001004$. These results
demonstrate a reduced mechanism and do not calibrate aluminum.

\section{Requirements for quantitative aluminum use}

The active two-row branch is not a quantitative aluminum law. Quantitative use requires a
...
for dislocation storage and cyclic hardening. Until then it remains an active
ideal single-registry mechanism with explicit limitations.
