...
def test_n3_gibbs_tensor_tt_prototype_preserves_mass_and_accuracy():
    result = compress_initial_gibbs_6d(
        model_params=ModelParams(
            n_cells=3,
            # This verification temperature resolves multiple interacting
            # cells on the deliberately coarse dense grid. At kT=0.03 the
            # discrete Gibbs mass is effectively a one-cell delta and an
            # accurate rank-one TT is the expected numerical result.
            kT=0.16,
            mobility_a=1.0,
            mobility_s=0.05,
    ...
    assert float(result["product_closure_l1_error"]) > 1.0e-5
